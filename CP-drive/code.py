...
while True:
	...
	long_reading = '{:d}\t{:5.3f}\t{:6.3f}\t{:6.3f}\t{:5.1f}'.format(time.time(), Voltage(ref), Voltage(rms), arcs, battery_monitor.cell_percent)
	...
	if DipSW['3'].value == False: ## ref signal > 100mV, arcs < 20
		...
		try:
			...
			if s%60 == 0:
				mqtt_client.publish(sssm_bat_feed, battery_monitor.cell_percent)
