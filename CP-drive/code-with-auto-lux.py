...
battery_monitor.pack_size = PackSize.MAH400   # MAH1200 ???
...
#		time.sleep(0.5)
	...
	return lux, mag, visible, infrared, full_spectrum
...
while True:
	...
	if lux_read:
		lux, mag, vix, ir, full = LUXsensor_read()
	...
	if arcs < 10.0 and ref > 2000: # ref voltage > 200mV
		...
		if lux_read:
			lux, mag, vix, ir, full = LUXsensor_read()
			status_reading = '{:.1f}M'.format(mag)
		        reading_last='LUX {:8.2f}'.format(lux)
		else:
			status_reading = 'SSSM *"'
	...
	if DipSW['3'].value == False: ## ref signal > 100mV, arcs < 20
        	# Firecapture PlugIn Compatible
                ...
                print ('A1: {:4.2f}'.format(arcs)) # RMS Variation Value / Seeing
	...
	if lux_read:
		print ('LUX: {:6.2f}'. format(lux))
		print ('MAG: {:6.2f}'. format(mag))
